...
export class Edge {
    ...
    // neighboring face).
    //
    constructor(vi1: number, vi2: number, f: Face, id: number, owner: CGObject) {
    ...
    //
    vertex(i: number): Vertex | null {
        if (i == 0) {
            return this.owner.vertex(this.sourcei);
        } else if (i == 1) {
            return this.owner.vertex(this.targeti);
        } else {
            return null;
        }
    }
    ...
    //
    position(i: number): Point3d | null {
        if (i > 1) {
            return null;
        } else {
            return this.vertex(i)!.position;
        }
    }
    ...
    // surface.  If two, then this edge acts as a hinge between two
    // faces.
    //
    faces(): Face[] {
        if (this.twin == null) {
...
export type PlacementType = { position: Point3d, scale: number, orientation: number }
...
export class Vertex {
    ...
    // with the given id.
    //
    constructor(P: Point3d, id: number) {
        this.position = P;
        this.id       = id;
        this.normal   = null;
    }

    getRelativePosition(placement: PlacementType): Point3d {
        /*
...
export class Face {
    ...
    //
    // new Face(vi1, vi2, vi3, id, owner)
    //
    // Create and initialize a new face instance.
    //
    // Instance attributes:
    //
    //   * vi1,vi2,vi3: vertex ids that form the face
    //   * fn: face normal
    //   * id: integer id identifying this vertex
    //   * owner: the CGObject of the face
    //
    constructor(vi1: number, vi2: number, vi3: number, id: number, owner: CGObject) {
        this.id = id;
    ...
    vertex(i: number): Vertex | null {
        if (i > 2) {
            ...
            return this.owner.vertex(this.vertexi[i]);
        }
    }
...
export class CGObject {
    ...
    buildFromOBJ(objText: string, isFlipped: boolean) {
        /*
    ...
    addFace(vi1: number, vi2: number, vi3: number) {
        console.assert(!this.lock, "Object surface is read-only!");
        
        const fi = this.faces.length;
        const f = new Face(vi1, vi2, vi3, fi, this);
    ...
    //
    edgeName(vi1: number, vi2: number): string {
    ...
    // indices.
    //
    getEdge(vi1: number, vi2: number): Edge {
    ...
    //
    addEdge(vi1: number, vi2: number, f: Face) {
        console.assert(!this.lock, "Object surface is read-only!");
        ...
        if (this.edgeMap.has(enm)) {
            ...
            const e = new Edge(vi1,vi2,f,ei,this);
    ...
    //
    read(objText: string, isFlipped: boolean) {
