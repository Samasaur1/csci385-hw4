...
const OFF_CAMERA_COLOR = {r:0.325, g:0.375, b:0.075}; // Chalk orange.
...
//
const bunny = new Placement("bunny",new Point3d(0.72,-0.166,0.0));
bunny.resize(0.254,null);
bunny.rotateBy(-25.3);
//
const cube = new Placement("cube",new Point3d(1.333,0.2,0.0));
cube.resize(0.36,null);
cube.rotateBy(35);
//
gWalkThru.placements = [bunny,cube];
...
let gPlacing       = false; // User has selected and is placing an object?
let gPlacement     = null;  // Object we are currently placing, or null.
...
function handlePlacement(mouseXY, down, drag) {
     ...
     * gets placed in the scene, or else a nearby one is selected.
     * This puts the GUI in CLONE_SELECTED mode. If this is followed
     * by a dragging of the mouse, then this code checks to see
     * whether the movement extends beyond a certain radius. If so,
     * it enters CLONE_SCALE mode to resize the object.  If not, and
     * the mouse button is released, it enters CLONE_LOCATE mode so
     * that the clone be moved around. A later click in this mode
...
function drawShotPreview() {
...

    {
        ...
        glScissor(gWidth-gHeight,0,gHeight,gHeight); // Limit where drawn.
        for (let placement of gWalkThru.placements) {
            // Draw each object.
...
function drawCameraPath() {
    ...
    const smooth_shots = gWalkThru.smooth_shots();
    for (let index = 1; index < smooth_shots.length; index++) {
        const shot0 = smooth_shots[index-1];
        const shot1 = smooth_shots[index];

        //
        // Compute path line.
        const position0 = shot0.position;
        const position1 = shot1.position;
        const heading = position1.minus(position0).unit();
...
function draw() {
    ...
    glEnable(GL_SCISSOR_TEST);
    glScissor(0, 0, gWidth - gHeight, gHeight); // Limit where drawn.
    drawScene();
    drawCameraPath();
    glDisable(GL_SCISSOR_TEST);
    //
...
function makeFloor(document) {
    /*
     * Makes floor marks on the top portion of the
    ...
    // Middle hash marks.
    for (; i <= 54; i += 2) {
        document.line(2+i,2,i,4);
    }
    // Top and bottom lines.
    document.line(2,2-16,56,2-16);
    document.line(2,2-16+18,56,2-16+18);
}

function makeBinding(document) {
    /*
     * Makes binding marks on the bottom portion of the
     * flip-book PDF. It is credit card-sized, so 54mm x 86mm.
     */
    document.setLineWidth(0.2);
    document.setDrawColor(128,128,128);
    ...
    for (; i <= 18; i += 3) {
        document.line(2+i,58,2,58+i);
    }
    // Middle hash marks.
    for (; i <= 54; i += 3) {
        document.line(2+i,58,2+i-18,58+18);
    }
    // Lower right hash marks.
    for (i = 3; i <= 18; i += 3) {
        document.line(56, 58 + i, 56 - 18 + i, 58 + 18);
    }
    // Top and bottom lines.
    document.line(2,58,56,58);
    document.line(2,58+18,56,58+18);
}

function makeCropMarks(document) {
    /*
    ...
    document.line(2,0,2,90);
    document.line(0,88,58,88);
    document.line(0,2,58,2);
    document.line(56,0,56,90);
}    
...
function staple(document) {
    ...
    document.setDrawColor(50,50,50);
    document.line(11,58+24,23,58+24);    // Left top.
    document.line(11,58+24,11,58+25);
    document.line(23,58+24,23,58+25);
    document.setDrawColor(150,150,150);
    document.line(11,58+25,11,58+26);     // Left tines.
    document.line(23,58+25,23,58+26);
    document.line(11,58+26,16,58+25);
    document.line(23,58+26,18,58+25);
    
    document.setDrawColor(50,50,50);
    document.line(35,58+24,47,58+24);   // Right top.
    document.line(35,58+24,35,58+25);
    document.line(47,58+24,47,58+25);
    document.setDrawColor(150,150,150);
    document.line(35,58+25,35,58+26);   // Right tines.
    document.line(47,58+25,47,58+26);
    document.line(35,58+26,40,58+25);
    document.line(47,58+26,42,58+25);
}
...
async function makePDF() {
     ...
     *
     */
    const doc = new jsPDF({unit:'mm', format:[58,90]});
    ...
    doc.text("CSCI 385 F24",29, 17, null, null, "center");
